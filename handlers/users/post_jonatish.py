...
@dp.message_handler(state=Forma.psot_video, content_types=ContentType.VIDEO)  # if
async def bot_echo(message: types.Message, state: FSMContext):
...
@dp.message_handler(state=Forma.caption_video)  # elif
async def bot_echo(message: types.Message, state: FSMContext):
...
@dp.message_handler(state=Forma.post_photo, content_types=ContentType.PHOTO)  # if
async def bot_echo(message: types.Message, state: FSMContext):
...
@dp.message_handler(state=Forma.caption_photo)  # elif
async def bot_echo(message: types.Message, state: FSMContext):
    caption = message.text
